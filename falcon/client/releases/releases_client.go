...
// New creates a new releases API client with basic auth credentials.
...
// New creates a new releases API client with a bearer token for authentication.
// It takes the following parameters:
// - host: http host (github.com).
// - basePath: any base path for the API client ("/v1", "/v3").
// - scheme: http scheme ("http", "https").
...
func (a *Client) CombinedReleasesV1Mixin0(params *CombinedReleasesV1Mixin0Params, opts ...ClientOption) (*CombinedReleasesV1Mixin0OK, error) {
	...
	if params == nil {
		params = NewCombinedReleasesV1Mixin0Params()
	}
	op := &runtime.ClientOperation{
		ID:                 "CombinedReleasesV1Mixin0",
		Method:             "GET",
	...
	success, ok := result.(*CombinedReleasesV1Mixin0OK)
	if ok {
		return success, nil
	}
	// unexpected success response
	// safeguard: normally, absent a default response, unknown success responses return an error above: so this is a codegen issue
	msg := fmt.Sprintf("unexpected success response for CombinedReleasesV1Mixin0: API contract not enforced by server. Client expected to get an error, but got: %T", result)
	panic(msg)
}
