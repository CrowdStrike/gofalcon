// Code generated by go-swagger; DO NOT EDIT.

package models

// This file was generated by the swagger tool.
// Editing this file might prove futile when you re-run the swagger generate command

import (
	"context"

	"github.com/go-openapi/errors"
	"github.com/go-openapi/strfmt"
	"github.com/go-openapi/swag"
	"github.com/go-openapi/validate"
)

// RegistrationAzureManagementGroupExternalV1 registration azure management group external v1
//
// swagger:model registration.AzureManagementGroupExternalV1
type RegistrationAzureManagementGroupExternalV1 struct {

	// default subscription id
	DefaultSubscriptionID string `json:"default_subscription_id,omitempty"`

	// Azure Tenant ID to use.
	// Required: true
	TenantID *string `json:"tenant_id"`
}

// Validate validates this registration azure management group external v1
func (m *RegistrationAzureManagementGroupExternalV1) Validate(formats strfmt.Registry) error {
	var res []error

	if err := m.validateTenantID(formats); err != nil {
		res = append(res, err)
	}

	if len(res) > 0 {
		return errors.CompositeValidationError(res...)
	}
	return nil
}

func (m *RegistrationAzureManagementGroupExternalV1) validateTenantID(formats strfmt.Registry) error {

	if err := validate.Required("tenant_id", "body", m.TenantID); err != nil {
		return err
	}

	return nil
}

// ContextValidate validates this registration azure management group external v1 based on context it is used
func (m *RegistrationAzureManagementGroupExternalV1) ContextValidate(ctx context.Context, formats strfmt.Registry) error {
	return nil
}

// MarshalBinary interface implementation
func (m *RegistrationAzureManagementGroupExternalV1) MarshalBinary() ([]byte, error) {
	if m == nil {
		return nil, nil
	}
	return swag.WriteJSON(m)
}

// UnmarshalBinary interface implementation
func (m *RegistrationAzureManagementGroupExternalV1) UnmarshalBinary(b []byte) error {
	var res RegistrationAzureManagementGroupExternalV1
	if err := swag.ReadJSON(b, &res); err != nil {
		return err
	}
	*m = res
	return nil
}

// String returns the JSON body of this registration azure management group external v1. It implements
// fmt.Stringer so that %v and %+v render the value instead of a pointer address.
func (m *RegistrationAzureManagementGroupExternalV1) String() string {
	if m == nil {
		return "<nil>"
	}
	b, err := swag.WriteJSON(m)
	if err != nil {
		return err.Error()
	}
	return string(b)
}
