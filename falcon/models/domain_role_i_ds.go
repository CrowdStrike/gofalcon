// Code generated by go-swagger; DO NOT EDIT.

package models

// This file was generated by the swagger tool.
// Editing this file might prove futile when you re-run the swagger generate command

import (
	"context"

	"github.com/go-openapi/errors"
	"github.com/go-openapi/strfmt"
	"github.com/go-openapi/swag"
	"github.com/go-openapi/validate"
)

// DomainRoleIDs domain role i ds
//
// swagger:model domain.RoleIDs
type DomainRoleIDs struct {

	// role ids
	// Required: true
	RoleIds []string `json:"roleIds"`
}

// Validate validates this domain role i ds
func (m *DomainRoleIDs) Validate(formats strfmt.Registry) error {
	var res []error

	if err := m.validateRoleIds(formats); err != nil {
		res = append(res, err)
	}

	if len(res) > 0 {
		return errors.CompositeValidationError(res...)
	}
	return nil
}

func (m *DomainRoleIDs) validateRoleIds(formats strfmt.Registry) error {

	if err := validate.Required("roleIds", "body", m.RoleIds); err != nil {
		return err
	}

	return nil
}

// ContextValidate validates this domain role i ds based on context it is used
func (m *DomainRoleIDs) ContextValidate(ctx context.Context, formats strfmt.Registry) error {
	return nil
}

// MarshalBinary interface implementation
func (m *DomainRoleIDs) MarshalBinary() ([]byte, error) {
	if m == nil {
		return nil, nil
	}
	return swag.WriteJSON(m)
}

// UnmarshalBinary interface implementation
func (m *DomainRoleIDs) UnmarshalBinary(b []byte) error {
	var res DomainRoleIDs
	if err := swag.ReadJSON(b, &res); err != nil {
		return err
	}
	*m = res
	return nil
}

// String returns the JSON body of this domain role i ds. It implements
// fmt.Stringer so that %v and %+v render the value instead of a pointer address.
func (m *DomainRoleIDs) String() string {
	if m == nil {
		return "<nil>"
	}
	b, err := swag.WriteJSON(m)
	if err != nil {
		return err.Error()
	}
	return string(b)
}
